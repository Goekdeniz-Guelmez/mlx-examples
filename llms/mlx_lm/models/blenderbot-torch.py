...
class BlenderbotLearnedPositionalEmbedding(nn.Embedding):
    ...
    def forward(self, input_ids_shape: mx.array, cache: int = 0):
        """`input_ids_shape` is expected to be [bsz x seqlen]."""
        bsz, seq_len = input_ids_shape[:2]
        positions = torch.arange(
            cache, cache + seq_len, dtype=torch.long, device=self.weight.device
        )
...
class BlenderbotDecoder(BlenderbotPreTrainedModel):
    ...
    def forward(
        ...
        # cache
        cache = past_key_values[0][0].shape[2] if past_key_values is not None else 0

        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids)

        attention_mask = _prepare_4d_causal_attention_mask(
            attention_mask, input_shape, inputs_embeds, cache
        )
        ...
        # embed positions
        positions = self.embed_positions(input_shape, cache)
